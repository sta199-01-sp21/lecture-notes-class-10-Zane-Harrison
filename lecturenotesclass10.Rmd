...
The probability is 43/84, or roughly 52%.

- What is the probability a randomly selected patient received the placebo
  and had a marked improvement?
The probability is 7/84, or roughly 8.3%.
  
- What is the probability a randomly selected patient received the placebo
  and the treatment?
The probability is 0 because these are mutally exclusive.
  
- What is the probability a randomly selected patient had some improvement or
  was on the treatment?
The probability is 48/84, or roughly 57%.
...
How about white?
The probability that the marble is blue is 3/8, and the probability that the 
marble is white is 1/8.
...
Pretty close, and it gets closer as the sample size increases.
...
My guess is that its more likely that the absolute value of the difference will 
be equal to 4.
...
two_roll_results <- tibble(
  ...
  roll_2 = sample(dice_roll, size = 10000, replace = TRUE)
)
two_roll_results
```
...
```{r identify_events}
dice_roll <- dice_roll %>%
  mutate(condition1 = roll_1 ^ 2 == roll_2) %>%
  mutate(condition2 = abs(roll_1 - roll_2 == 4))
two_roll_results
```

```{r summarize_rolls}
two_roll_results %>%
  summarise(event_1_prob = mean(condition1), event_2_prob = mean(condition2))
```
